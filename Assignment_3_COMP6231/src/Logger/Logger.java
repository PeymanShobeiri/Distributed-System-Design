    ...
    private static String getFileName(String ID, int logType) {
        ...
        if (logType == LOG_TYPE_SERVER) {
            switch (ID.toUpperCase()) {
                case "NYK":
                    fileName += "/logs/servers/Server_logs_NewYork.txt";
                    break;
                case "LON":
                    fileName += "/logs/servers/Server_logs_London.txt";
                    break;
                case "TOK":
                    fileName += "/logs/servers/Server_logs_Tokyo.txt";
                    break;
                default:
                    fileName += "/logs/servers/Server_logs_UnknownServer.txt";
            }
        } else {
            fileName += "/logs/client/Client_logs_" + ID + ".txt";
        }
        return fileName;
    }
